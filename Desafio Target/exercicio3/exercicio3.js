...
function analisarFaturamento() {
    fetch('faturamento.json') // Carrega o arquivo JSON
        ...
        .then(faturamentoDiario => {
            ...
            // Exibe os resultados com formatação correta
            document.getElementById('menor').textContent = `Menor valor de faturamento: ${formatarReais(menorFaturamento)}`;
            document.getElementById('maior').textContent = `Maior valor de faturamento: ${formatarReais(maiorFaturamento)}`;
